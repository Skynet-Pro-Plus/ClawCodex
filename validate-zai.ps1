...
<#
.SYNOPSIS
  Terminal-friendly Z.ai API key check for ClawCodex onboarding.

.DESCRIPTION
  - If no key or placeholder: prints status and exits 10.
  - If a key is set: GET https://open.bigmodel.cn/api/paas/v4/models with Bearer auth.
...
function Get-ZaiApiKey {
    $fileKey = $null
    if ($fileVars.ContainsKey("ZAI_API_KEY")) {
        $fileKey = $fileVars["ZAI_API_KEY"].Trim()
    }
    $procKey = [Environment]::GetEnvironmentVariable("ZAI_API_KEY", "Process")
    ...
    if (-not (Is-PlaceholderKey $fileKey)) {
        if ((-not (Is-PlaceholderKey $procKey)) -and ($fileKey -ne $procKey)) {
            Write-Host "  NOTE: ZAI_API_KEY is set in both .env and this process; using .env for validation." -ForegroundColor DarkYellow
        }
        return $fileKey
    }
...
function Get-ZaiBaseUrl {
    $fileBase = $null
    if ($fileVars.ContainsKey("ZAI_BASE_URL")) {
        $fileBase = $fileVars["ZAI_BASE_URL"].Trim()
    }
    $procBase = [Environment]::GetEnvironmentVariable("ZAI_BASE_URL", "Process")
...
$apiKey = Get-ZaiApiKey
$baseUrl = Get-ZaiBaseUrl

Write-Host ""
Write-Host "  === Z.ai (terminal check) ===" -ForegroundColor Cyan

if (Is-PlaceholderKey $apiKey) {
    Write-Host "  STATUS: No real Z.ai API key yet (missing or still a placeholder)." -ForegroundColor Yellow
    Write-Host "  NEXT:    Enter a valid Z.ai key in this terminal when prompted." -ForegroundColor Yellow
    Write-Host ""
    exit 10
}

$apiKeySource = Get-ZaiApiKeySource
...
try {
    ...
    Write-Host "  STATUS: Key accepted by Z.ai (GET /models OK)." -ForegroundColor Green
    ...
    if ($status -eq 401 -or $status -eq 403) {
        Write-Host "  STATUS: Z.ai rejected this key (HTTP $status). It is set but not valid." -ForegroundColor Red
        Write-Host "  FIX:    Enter a valid Z.ai key in this terminal when prompted." -ForegroundColor Red
        Write-Host ""
        exit 2
    }

    Write-Host "  STATUS: Z.ai did not return a usable response for this key (network or HTTP error)." -ForegroundColor DarkYellow
