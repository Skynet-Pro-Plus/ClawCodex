...
/// Maximum time to wait for the next SSE chunk after the stream has started.
...
impl MessageStream {
    ...
    pub async fn next_event(&mut self) -> Result<Option<StreamEvent>, ApiError> {
        loop {
            ...
            let chunk_result = tokio::time::timeout(CHUNK_IDLE_TIMEOUT, self.response.chunk())
                .await
                .map_err(|_| {
                    ApiError::Io(std::io::Error::new(
                        std::io::ErrorKind::TimedOut,
                        format!(
                            "stream idle for {}s without a chunk or [DONE] — \
                             provider may have hit its output-token limit silently; \
                             try a shorter prompt or a smaller max_tokens value",
