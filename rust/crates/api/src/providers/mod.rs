...
pub fn detect_provider_kind(model: &str) -> ProviderKind {
    ...
    // Prefer OpenAI-compat (OpenRouter, OpenAI, local OpenAI servers) over
    // Anthropic when both credential sets exist but the model name is
    // ambiguous — avoids Anthropic "stealing" routing for OpenRouter users
    // who also have ANTHROPIC_API_KEY in the environment.
    ...
    model_token_limit(model).map_or_else(
        || {
            ...
            if canonical.contains("opus") {
                32_000
            } else if canonical.contains("minimax") {
                8_192
            } else {
                64_000
            }
...
";
    ...
    #[test]
    fn openai_api_key_routes_openai_before_anthropic_for_unknown_model() {
        // Unknown model string + both Anthropic and OpenAI-compat credentials:
        // OpenRouter / OpenAI users must not lose routing to Anthropic.
        let _lock = env_lock();
        let _anthropic = EnvVarGuard::set("ANTHROPIC_API_KEY", Some("sk-ant-test"));
        let _anthropic_token = EnvVarGuard::set("ANTHROPIC_AUTH_TOKEN", None);
        let _openai_key = EnvVarGuard::set("OPENAI_API_KEY", Some("sk-openai-test"));
        let _openai_base = EnvVarGuard::set("OPENAI_BASE_URL", None);
        let _xai = EnvVarGuard::set("XAI_API_KEY", None);

        assert_eq!(
            detect_provider_kind("custom-local-model-id"),
            ProviderKind::OpenAi
        );
    }
