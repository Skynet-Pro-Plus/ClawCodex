...
use runtime::{
    check_base_commit, format_stale_base_warning, format_usd, kill_background_processes,
...
/// interactive `[y/N]` permission prompts are skipped and every approval request is allowed.
static AUTO_APPROVE_PERMISSIONS: AtomicBool = AtomicBool::new(false);
        ...
        CliAction::Repl {
            ...
            if openrouter_picker::should_run_openrouter_repl_model_picker(model_explicit) {
                match openrouter_picker::run_openrouter_model_picker() {
                    ...
                    Err(error) => {
                        eprintln!("Warning: OpenRouter model picker failed: {error}");
                        eprintln!(
                            "Warning: falling back to `{}`",
                            openrouter_picker::OPENROUTER_DEFAULT_MODEL_ID
                        );
                        model = resolve_model_alias_with_config(
                            openrouter_picker::OPENROUTER_DEFAULT_MODEL_ID,
                        );
                    }
                }
...
enum CliAction {
    ...
    Repl {
        ...
        /// True when `--model` / `--model=` was supplied on the command line.
        model_explicit: bool,
    },
...
fn parse_args(args: &[String]) -> Result<CliAction, String> {
    AUTO_APPROVE_PERMISSIONS.store(false, Ordering::Relaxed);
    ...
    let mut model_context_window: Option<u32> = None;
    ...
    while index < args.len() {
        match args[index].as_str() {
            ...
            "--auto-approve-permissions" => {
                AUTO_APPROVE_PERMISSIONS.store(true, Ordering::Relaxed);
                index += 1;
            }
            "--compact" => {
                compact = true;
            ...
            "--model-context-window" => {
                let value = args
                    .get(index + 1)
                    .ok_or_else(|| "missing value for --model-context-window".to_string())?;
                let parsed: u32 = value
                    .trim()
                    .parse()
                    .map_err(|_| format!("invalid --model-context-window: '{value}'"))?;
                if parsed == 0 {
                    return Err("--model-context-window must be greater than 0".to_string());
                }
                model_context_window = Some(parsed);
                index += 2;
            }
            flag if flag.starts_with("--model-context-window=") => {
                let value = flag
                    .strip_prefix("--model-context-window=")
                    .ok_or_else(|| "invalid --model-context-window flag".to_string())?;
                let parsed: u32 = value
                    .trim()
                    .parse()
                    .map_err(|_| format!("invalid --model-context-window: '{value}'"))?;
...
fn resolve_repl_model(cli_model: String) -> String {
    ...
    if !openrouter_stack_configured() {
...
fn build_runtime_with_plugin_state(
    ...
    let merged_completion_verify = runtime::merge_completion_verify_workspace_overlay(
        feature_config.completion_verify().clone(),
...
impl runtime::PermissionPrompter for CliPermissionPrompter {
    fn decide(
        ...
        if AUTO_APPROVE_PERMISSIONS.load(Ordering::Relaxed)
            || env::var("CLAW_AUTO_APPROVE_PERMISSIONS").ok().as_deref() == Some("1")
        {
...
fn format_bash_result(icon: &str, parsed: &serde_json::Value) -> String {
        ...
        .and_then(|value| value.as_str())
    {
        ...
        if parsed
            ...
            .and_then(|value| value.as_str())
            == Some("background.launch_verified")
        {
            write!(&mut lines[0], " verified-start").expect("write to string");
        }
...
fn print_help_to(out: &mut impl Write) -> io::Result<()> {
    ...
    writeln!(
        out,
        "  --model-context-window N   Model context size in tokens (auto-compact / pre-send guard)"
    )?;
    writeln!(
        out,
    ...
    writeln!(
        out,
        "  --auto-approve-permissions     Auto-allow y/N permission prompts (or CLAW_AUTO_APPROVE_PERMISSIONS=1)"
    )?;
    ...
    writeln!(out)?;
    writeln!(out, "Portable OpenRouter:")?;
    writeln!(
        out,
        "  A .env beside the executable (or in the working directory) can set OPENAI_API_KEY and OPENAI_BASE_URL."
    )?;
    writeln!(
        out,
        "  When OpenRouter is configured, the REPL shows a model picker unless you pass --model or set CLAW_SKIP_OPENROUTER_MODEL_PICKER=1."
    )?;
    writeln!(out)?;
...
mod tests {
    ...
    #[test]
    fn repl_with_explicit_model_flag_sets_model_explicit() {
        let _guard = env_lock();
        std::env::remove_var("RUSTY_CLAUDE_PERMISSION_MODE");
        std::env::remove_var("CLAW_MODEL_CONTEXT_WINDOW");
        let args = vec![
    ...
    fn resolve_repl_model_falls_back_to_anthropic_model_env_when_default() {
        ...
        let prior_openai_base = std::env::var_os("OPENAI_BASE_URL");
    ...
    #[test]
    fn resolve_repl_model_skips_anthropic_model_env_when_openrouter_configured() {
        let _guard = env_lock();
        let root = temp_dir();
        fs::create_dir_all(&root).expect("root dir");
        let config_home = root.join("config");
        ...
        let prior_openai_key = std::env::var_os("OPENAI_API_KEY");
        let prior_openai_base = std::env::var_os("OPENAI_BASE_URL");
        std::env::set_var("OPENAI_API_KEY", "sk-or-test");
