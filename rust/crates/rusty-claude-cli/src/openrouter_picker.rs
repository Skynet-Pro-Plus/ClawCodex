...
fn openrouter_model_passes_claw_filters(raw: &Value) -> bool {
    if model_is_audio_specialized(raw) {
        return false;
    }
    ...
    if let Some(out_mods) = raw
        ...
        .and_then(Value::as_array)
    {
        if !out_mods.is_empty() && !out_mods.iter().all(|v| v.as_str() == Some("text")) {
            return false;
        }
    }
...
fn model_is_audio_specialized(raw: &Value) -> bool {
    let mut haystack = String::new();
    for pointer in ["/id", "/name", "/architecture/modality"] {
    ...
    if audio_terms.iter().any(|term| haystack.contains(term)) {
        return true;
    }
        ...
        if raw
            ...
            .is_some_and(|items| {
                items
                    ...
                    .any(|item| item.eq_ignore_ascii_case("audio"))
            })
        {
            return true;
        }
...
mod tests {
    ...
    fn filters_for_tool_text_and_large_context_models() {
        ...
        let mut mixed_output = compatible_model();
        mixed_output["architecture"]["output_modalities"] = json!(["text", "audio"]);
        assert!(!openrouter_model_passes_claw_filters(&mixed_output));
    }

    #[test]
    fn filters_audio_tts_and_stt_models() {
        let mut audio_input = compatible_model();
        audio_input["architecture"]["input_modalities"] = json!(["text", "audio"]);
        assert!(!openrouter_model_passes_claw_filters(&audio_input));

        let mut tts_modality = compatible_model();
        tts_modality["architecture"]["modality"] = json!("text->audio");
        assert!(!openrouter_model_passes_claw_filters(&tts_modality));

        let mut whisper = compatible_model();
        whisper["id"] = json!("openai/whisper-large-v3");
        whisper["name"] = json!("Whisper Large v3");
        assert!(!openrouter_model_passes_claw_filters(&whisper));

        let mut transcriber = compatible_model();
        transcriber["id"] = json!("example/speech-to-text-pro");
        transcriber["name"] = json!("Speech to Text Pro");
        assert!(!openrouter_model_passes_claw_filters(&transcriber));
    }
