...
/// WSL exposes Windows drives as `/mnt/d/...`. Map to `D:\...` on Windows so
...
#[cfg(not(windows))]
fn wsl_mnt_drive_path_to_windows(_path: &str) -> Option<PathBuf> {
    None
}
...
fn repair_windows_double_drive_segment(path: &str) -> Option<PathBuf> {
    let trimmed = path.trim_start_matches('\u{feff}');
    let (verbatim, rest) = if let Some(r) = trimmed.strip_prefix(r"\\?\") {
        (true, r)
    } else {
        (false, trimmed)
    };
    ...
    let drive_byte = bytes[0].to_ascii_uppercase();
    let drive = drive_byte as char;
    let after_prefix = rest.get(3..)?;
    ...
    let letter = first.as_bytes()[0];
    if !letter.eq_ignore_ascii_case(&drive_byte) {
        return None;
    }
    let tail = after_prefix.get(seg_end..).unwrap_or("");
    let rebuilt = format!("{drive}:{tail}");
    let with_prefix = if verbatim {
        format!(r"\\?\{rebuilt}")
    } else {
        rebuilt
    };
    Some(PathBuf::from(with_prefix))
}

#[cfg(not(windows))]
fn repair_windows_double_drive_segment(_path: &str) -> Option<PathBuf> {
    None
}

/// Git Bash / MSYS2 expose Windows drives as `/d/...` instead of `D:\...`.
/// Rust's `Path` helpers treat `/d/...` inconsistently on Windows (paths can
/// degrade into `D:\d\...`), so map this prefix before joining or canonicalizing.
fn unix_msys_style_drive_path_to_windows(path: &str) -> Option<PathBuf> {
    let path = path.trim_start_matches('\u{feff}');
...
fn resolve_path_candidate(path: &str) -> io::Result<PathBuf> {
    if let Some(repaired) = repair_windows_double_drive_segment(path) {
        return Ok(repaired);
    }
    if let Some(mapped) = unix_msys_style_drive_path_to_windows(path) {
        return Ok(mapped);
    }
    if let Some(mapped) = wsl_mnt_drive_path_to_windows(path) {
        return Ok(mapped);
    }
...
fn normalize_path(path: &str) -> io::Result<PathBuf> {
    let candidate = resolve_path_candidate(path)?;
...
mod tests {
    ...
    #[test]
    fn maps_typical_git_bash_user_style_path() {
        let mapped = super::unix_msys_style_drive_path_to_windows("/c/Users/dev/readme.md")
            .expect("msys-style user path should map");
        let normalized = mapped.to_string_lossy().replace('\\', "/");
        assert_eq!(normalized, "C:/Users/dev/readme.md");
    }

    #[test]
    fn ignores_non_msys_unix_paths() {
        assert!(super::unix_msys_style_drive_path_to_windows("/usr/bin").is_none());
        assert!(super::unix_msys_style_drive_path_to_windows("foo/bar.txt").is_none());
    }
    ...
    #[cfg(windows)]
    #[test]
    fn maps_wsl_mnt_uppercase_prefix() {
        let mapped = super::wsl_mnt_drive_path_to_windows("/MNT/c/Users/dev/readme.md")
            .expect("uppercase MNT prefix should map");
        let normalized = mapped.to_string_lossy().replace('\\', "/");
        assert_eq!(normalized, "C:/Users/dev/readme.md");
    }
    ...
    #[cfg(windows)]
    #[test]
    fn does_not_repair_valid_single_letter_segment() {
        assert!(super::repair_windows_double_drive_segment(r"D:\x\foo.txt").is_none());
    }
