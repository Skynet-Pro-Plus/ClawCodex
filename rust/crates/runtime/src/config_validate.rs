...
const TOP_LEVEL_FIELDS: &[FieldSpec] = &[
    ...
    FieldSpec {
        name: "completionVerify",
        expected: FieldType::Object,
    },
];

const COMPLETION_VERIFY_FIELDS: &[FieldSpec] = &[
    FieldSpec {
        name: "enabled",
...
pub fn validate_config_file(
    ...
    if let Some(cv) = object
        .get("completionVerify")
        .and_then(JsonValue::as_object)
    {
        result.merge(validate_object_keys(
            cv,
            COMPLETION_VERIFY_FIELDS,
            "completionVerify",
            source,
            &path_display,
        ));
...
mod tests {
  ...
  "sandbox": {"enabled": false}
    ...
    #[test]
    fn completion_verify_nested_keys_validate() {
        let source = r#"{"completionVerify": {"enabled": true, "command": "cargo check", "timeoutMs": 120000, "skipIfNoTools": true}}"#;
        let parsed = JsonValue::parse(source).expect("valid json");
        let object = parsed.as_object().expect("object");
        let result = validate_config_file(object, source, &test_path());
