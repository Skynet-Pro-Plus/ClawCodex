...
/// Optional shell command the engine runs after the model stops requesting tools,
/// before the turn is considered complete. If the command exits non-zero (or times out),
/// a synthetic user message is injected so the model must fix issues and try again.
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct CompletionVerifyConfig {
    pub enabled: bool,
    pub command: String,
    pub timeout_ms: u64,
    pub skip_if_no_tools: bool,
}

impl Default for CompletionVerifyConfig {
    fn default() -> Self {
        Self {
            enabled: false,
            command: String::new(),
            timeout_ms: 600_000,
            skip_if_no_tools: true,
        }
    }
}
...
impl ConfigLoader {
    ...
    pub fn load(&self) -> Result<RuntimeConfig, ConfigError> {
        ...
        let feature_config = RuntimeFeatureConfig {
            ...
            completion_verify: parse_optional_completion_verify_config(&merged_value)?,
        };
...
impl RuntimeConfig {
    ...
    #[must_use]
    pub fn completion_verify(&self) -> &CompletionVerifyConfig {
        self.feature_config.completion_verify()
    }
}
...
impl RuntimeFeatureConfig {
    ...
    #[must_use]
    pub fn with_completion_verify(mut self, completion_verify: CompletionVerifyConfig) -> Self {
        self.completion_verify = completion_verify;
        self
    }
...
fn parse_optional_completion_verify_config(
    root: &JsonValue,
) -> Result<CompletionVerifyConfig, ConfigError> {
    let mut cfg = CompletionVerifyConfig::default();
    let Some(object) = root.as_object() else {
        apply_completion_verify_env_overrides(&mut cfg);
        return Ok(cfg);
    };
    if let Some(raw) = object.get("completionVerify") {
        let verify = expect_object(raw, "merged settings.completionVerify")?;
        cfg.enabled =
            optional_bool(verify, "enabled", "merged settings.completionVerify")?.unwrap_or(false);
        if let Some(cmd) = optional_string(verify, "command", "merged settings.completionVerify")? {
            cfg.command = cmd.to_string();
        }
        if let Some(ms) = optional_u64(verify, "timeoutMs", "merged settings.completionVerify")? {
            cfg.timeout_ms = ms;
        }
        if let Some(skip) =
            optional_bool(verify, "skipIfNoTools", "merged settings.completionVerify")?
        {
...
fn apply_completion_verify_env_overrides(cfg: &mut CompletionVerifyConfig) {
    // Respect an explicit user choice: don't force-enable if they set enabled=false.
    if !cfg.enabled {
        return;
    }
    if let Ok(cmd) = std::env::var("CLAW_COMPLETION_VERIFY_CMD") {
        let trimmed = cmd.trim();
        if !trimmed.is_empty() {
            cfg.command = trimmed.to_string();
        }
    }
    if let Ok(ms) = std::env::var("CLAW_COMPLETION_VERIFY_TIMEOUT_MS") {
...
/// Merge `completionVerify` from `workspace_root/.claw/settings.json` over `base`, then re-apply
/// env overrides (which still respect `enabled: false`).
#[must_use]
pub fn merge_completion_verify_workspace_overlay(
    mut base: CompletionVerifyConfig,
    workspace_root: Option<&Path>,
) -> CompletionVerifyConfig {
    if let Some(ws) = workspace_root {
        let path = ws.join(".claw").join("settings.json");
        if let Ok(Some(parsed)) = read_optional_json_object(&path) {
            if let Err(error) = overlay_completion_verify_from_root(
                &mut base,
                &parsed.object,
                &path.display().to_string(),
            ) {
                eprintln!(
                    "warning: could not merge workspace completionVerify from {}: {error}",
                    path.display()
                );
            }
        }
    }
    apply_completion_verify_env_overrides(&mut base);
    base
}

fn overlay_completion_verify_from_root(
    cfg: &mut CompletionVerifyConfig,
    root: &BTreeMap<String, JsonValue>,
    context: &str,
) -> Result<(), ConfigError> {
    let Some(raw) = root.get("completionVerify") else {
        return Ok(());
    };
    let verify = expect_object(raw, &format!("{context}.completionVerify"))?;
    if let Some(enabled) = optional_bool(verify, "enabled", &format!("{context}.completionVerify"))?
    {
        cfg.enabled = enabled;
    }
    if let Some(cmd) = optional_string(verify, "command", &format!("{context}.completionVerify"))? {
        cfg.command = cmd.to_string();
    }
...
mod tests {
    use super::{
        deep_merge_objects, merge_completion_verify_workspace_overlay, parse_permission_mode_label,
        CompletionVerifyConfig, ConfigLoader, ConfigSource, McpServerConfig, McpTransport,
    ...
    #[test]
    fn merge_workspace_completion_verify_disables_over_repo_defaults() {
        let ws = temp_dir();
        fs::create_dir_all(ws.join(".claw")).expect("mkdir");
        fs::write(
            ws.join(".claw").join("settings.json"),
            r#"{"completionVerify":{"enabled":false,"command":""}}"#,
        )
        .expect("write overlay");
        let base = CompletionVerifyConfig {
            enabled: true,
            command: "cargo check --workspace".to_string(),
            timeout_ms: 1234,
            skip_if_no_tools: true,
        };
        let merged = merge_completion_verify_workspace_overlay(base, Some(ws.as_path()));
        assert!(!merged.enabled);
        fs::remove_dir_all(&ws).ok();
    }

    #[test]
    fn merge_workspace_disabled_skips_env_verify_cmd() {
        let prior = std::env::var("CLAW_COMPLETION_VERIFY_CMD").ok();
        std::env::set_var("CLAW_COMPLETION_VERIFY_CMD", "echo probe");
        let ws = temp_dir();
        fs::create_dir_all(ws.join(".claw")).expect("mkdir");
        fs::write(
            ws.join(".claw").join("settings.json"),
            r#"{"completionVerify":{"enabled":false}}"#,
        )
        .expect("write overlay");
        let merged = merge_completion_verify_workspace_overlay(
            CompletionVerifyConfig {
                enabled: true,
                command: String::new(),
                ..Default::default()
        ...
        assert!(!merged.enabled);
        match prior {
            Some(value) => std::env::set_var("CLAW_COMPLETION_VERIFY_CMD", value),
            None => std::env::remove_var("CLAW_COMPLETION_VERIFY_CMD"),
        }
        fs::remove_dir_all(&ws).ok();
    }
}
