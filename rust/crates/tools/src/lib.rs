...
pub fn mvp_tool_specs() -> Vec<ToolSpec> {
    vec![
        ...
        ToolSpec {
            name: "PowerShell",
            description: "Execute a PowerShell command with optional timeout. For Windows-native GUI apps or window launches, prefer this tool and set run_in_background to true so the process can keep running without hitting a foreground timeout.",
            input_schema: json!({
                "type": "object",
                "properties": {
                    "command": { "type": "string" },
...
struct TodoWriteOutput {
    ...
    /// Status transitions for todos that existed in the previous store (matched by content + activeForm).
    #[serde(rename = "statusChanges")]
    status_changes: Vec<TodoStatusChange>,
...
fn resolve_skill_path_in_skills_dir(
    ...
    // Support namespaced/system skill layouts like `.codex/skills/.system/<skill>/SKILL.md`.
    let namespace_direct = std::fs::read_dir(root).ok()?;
    for entry in namespace_direct.flatten() {
        let namespace_root = entry.path();
        if !namespace_root.is_dir() {
            continue;
        }
        let nested_direct = namespace_root.join(requested).join("SKILL.md");
    ...
    for entry in entries.flatten() {
        ...
        let nested_entries = std::fs::read_dir(entry.path()).ok()?;
        for nested_entry in nested_entries.flatten() {
            let nested_path = nested_entry.path();
            if !nested_path.is_dir() {
                continue;
            }
            let nested_skill_path = nested_path.join("SKILL.md");
            if !nested_skill_path.is_file() {
                continue;
            }
            if nested_entry
                .file_name()
                .to_string_lossy()
                .eq_ignore_ascii_case(requested)
                || skill_frontmatter_name_matches(&nested_skill_path, requested)
            {
                return Some(nested_skill_path);
            }
        }
    }
...
const DEFAULT_AGENT_MODEL: &str = "openai/gpt-4.1-mini";
...
fn resolve_repl_runtime(language: &str) -> Result<ReplRuntime, String> {
    match language.trim().to_ascii_lowercase().as_str() {
        "python" | "py" => resolve_python_repl_runtime(),
...
fn resolve_python_repl_runtime() -> Result<ReplRuntime, String> {
    ...
    for (program, args) in candidates {
        if command_exists(program) {
            return Ok(ReplRuntime { program, args });
        }
    }

    Err(String::from("python runtime not found"))
}
...
fn execute_shell_command(
    ...
    let effective_timeout_ms = timeout.map(|value| value.max(MIN_SHELL_TIMEOUT_MS));
    if run_in_background.unwrap_or(false) {
        ...
        if let Some(output) = verify_background_shell_launch(&mut child)? {
            let stdout = String::from_utf8_lossy(&output.stdout).trim().to_string();
            let stderr_text = String::from_utf8_lossy(&output.stderr);
            let stderr = background_launch_failure_message(&stderr_text, output.status.code());
...
fn verify_background_shell_launch(
    ...
    loop {
        if child.try_wait()?.is_some() {
            ...
            if let Some(mut handle) = child.stdout.take() {
                let _ = std::io::Read::read_to_end(&mut handle, &mut stdout);
            }
            let mut stderr = Vec::new();
            if let Some(mut handle) = child.stderr.take() {
                let _ = std::io::Read::read_to_end(&mut handle, &mut stderr);
            }
...
fn background_launch_failure_message(stderr: &str, exit_code: Option<i32>) -> String {
    ...
    if detail.is_empty() {
        match exit_code {
            ...
            None => "Background command exited before launch verification completed.".to_string(),
        }
    } else {
        format!("Background command exited before launch verification completed: {detail}")
    }
}
...
mod tests {
    ...
    #[test]
    fn skill_loads_system_namespaced_skill_prompt() {
        let _guard = env_guard();
        let home = temp_path("skills-system-home");
        let skill_dir = home
            .join(".codex")
            .join("skills")
            .join(".system")
        ...
        let result =
            execute_tool("Skill", &json!({ "skill": "imagegen" })).expect("Skill should succeed");

        let output: serde_json::Value = serde_json::from_str(&result).expect("valid json");
        let normalized_path = output["path"].as_str().expect("path").replace('\\', "/");
        assert!(normalized_path.ends_with(".codex/skills/.system/imagegen/SKILL.md"));
  ...
  "nbformat_minor": 5
    ...
    fn bash_tool_reports_success_exit_failure_timeout_and_background() {
        ...
        let timeout = execute_tool("bash", &json!({ "command": "sleep 6", "timeout": 10 }))
        ...
        let immediate_exit = execute_tool(
            "bash",
            &json!({ "command": "exit 9", "run_in_background": true }),
        )
        .expect("bash should report immediate background exit");
        ...
        assert!(immediate_exit_output["stderr"]
            .as_str()
            .expect("stderr")
...
"#,
        ...
        #[cfg(windows)]
        let original_pathext = std::env::var("PATHEXT").unwrap_or_default();
        #[cfg(windows)]
        ...
        assert_eq!(
            background_output["structuredContent"][0]["event"],
            "background.launch_verified"
        );
