...
<#
.SYNOPSIS
  Set or replace DEEPSEEK_API_KEY in repo-root .env from the CLI (masked input).
#>
param(
    [switch]$NoVerify
)
...
function Ensure-DotEnvFromExample {
    ...
    if (-not (Test-Path -LiteralPath $Example)) {
        @(
            ...
            "DEEPSEEK_BASE_URL=$DefaultDeepSeekBase",
...
Write-Host ""
Write-Host "  === Set DeepSeek API key (input hidden) ===" -ForegroundColor Cyan
...
Set-DotEnvValue -Lines $lines -Name "DEEPSEEK_API_KEY" -Value $plain -Found $hadKey
Set-DotEnvValue -Lines $lines -Name "DEEPSEEK_BASE_URL" -Value $DefaultDeepSeekBase -Found $hadBase
Set-DotEnvValue -Lines $lines -Name "CLAW_PROVIDER" -Value "deepseek" -Found $hadProvider
Set-DotEnvValue -Lines $lines -Name "CLAW_DEEPSEEK_MODEL" -Value $DefaultDeepSeekModel -Found $hadModel
Set-DotEnvValue -Lines $lines -Name "OPENAI_BASE_URL" -Value $DefaultDeepSeekBase -Found $hadOpenAiBase
...
$verifyVal = if ($verify) { ($verify -replace '^\s*DEEPSEEK_API_KEY\s*=\s*', '').Trim() } else { '' }
...
Write-Host ""
Write-Host ("  Saved DEEPSEEK_API_KEY to {0} (length {1}, fingerprint {2})" -f $DotEnv, $plain.Length, $mask) -ForegroundColor Green
...
if (-not $NoVerify) {
    ...
    $v = $LASTEXITCODE
    if ($v -eq 2) {
        Write-Host "  DeepSeek still rejected this key. Try again." -ForegroundColor Red
        exit 2
    }
    if ($v -ne 0 -and $v -ne 3) {
        Write-Host "  DeepSeek did not validate this key yet (validator exit $v)." -ForegroundColor Yellow
        exit $v
    }
}
